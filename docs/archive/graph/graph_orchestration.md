...
**Stable fingerprint scheme (hunk-level):**
Each hunk fingerprint must declare:
- `path`
- `base_ref`
...
- `worker_run_meta` (tool_profile_id, run_id, wave_id, budgets used)
...
### ResolverSpec
Maps capability → `{operationIds[] or families[], constraints_template}`.
Used by both planner and enforcer.
