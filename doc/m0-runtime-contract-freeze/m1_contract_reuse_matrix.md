...
| Capability discovery envelope | resource spec | Gateway (`dome`) | stable/v1 (new) | `domed` discovery/version negotiation | new | `Gateway.ListCapabilities` | include versions/schema/features fields | freeze in v1 | Required by `#61` target model. |
